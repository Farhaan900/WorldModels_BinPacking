...
from components.our_network import OurDQNNetwork, OurRainbowNetwork
...
class ParametricDQNAgent(dqn_agent.DQNAgent):
    ...
    def __init__(self,
                 ...
                 num_actions,
                 environment=None,
                 observation_shape=BIN_PACKING_SHAPE,
                 observation_dtype=atari_lib.NATURE_DQN_DTYPE,
                 stack_size=atari_lib.NATURE_DQN_STACK_SIZE,
                 network=OurDQNNetwork, 
                 gamma=0.99,
                 update_horizon=1,
    ...
    def _select_action(self):
        ...
        if random.random() <= epsilon:
            ...
            if self.environment:               
                possible_actions = np.where(self.environment.action_mask > np.zeros(self.num_actions))
                a= random.choice(possible_actions[0])
                return a
            else:
            ...
            if self.environment:
                ...
                a=np.argmax(masked_actions)
                return a
            else:
                a= tf.argmax(self._net_outputs.q_values, axis=1)[0]
                return a

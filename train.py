...
def worker(weights, seed, train_mode_int=1, max_len=-1):
    ...
    reward_list, t_list = simulate(model,
                                   train_mode=train_mode, render_mode=False, num_episode=num_episode, seed=seed,
                                   max_len=max_len)
    if batch_mode == 'min':
...
def master():
    ...
    while True:#Wait, how do we stop this?
        while True:
            ...
            h = (t, curr_time, avg_reward, r_min, r_max, std_reward, int(es.rms_stdev() * 100000) / 100000.,
            ...
            with open(filename, 'wt') as out:
                res = json.dump([np.array(es.current_param()).round(4).tolist()], out, sort_keys=True, indent=2,
                                separators=(',', ': '))
            ...
            if (t % eval_steps == 0):  # evaluate on actual task at hand
                ...
                curr_time = int(time.time()) - start_time
                best_record = [t, curr_time, "improvement", improvement, "curr", reward_eval, "prev",
                               prev_best_reward_eval, "best", best_reward_eval]
                ...
                sprint("Eval", t, curr_time, "improvement", improvement, "curr", reward_eval, "prev",
                       prev_best_reward_eval, "best", best_reward_eval)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--num_worker', type=int, default=58)
